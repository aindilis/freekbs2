package KBS2::ImportExport::Mod::LogicForm;
...
sub ConvertToInterlingua {
  ...
  } elsif ($args{InputType} eq "Logic Forms") {
    my $res = $self->ConvertLogicFormsToInterLingua
      (
       LogicForms => $args{Input},
      );
    if ($res->{Success}) {
      return {
	      Success => 1,
...
sub ConvertToOutputType {
  ...
  } elsif ($args{OutputType} eq "Logic Forms") {
    my $res = $self->ConvertInterlinguaToLogicForms
      (
...
sub ConvertLogicFormsToInterLingua {
  my ($self,%args) = @_;
  my @interlingua;
  # print Dumper({Args => \%args});
  foreach my $lf (@{$args{LogicForms}}) {
...
sub ConvertInterlinguaToLogicForms {
  my ($self,%args) = @_;
  return {
	  Success => 0,
	  Reasons => {
